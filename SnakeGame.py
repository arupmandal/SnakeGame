...
class startmenu():
    ...
    def mainloop(self):
        while 1:
            self.screen.fill((35,38,117))
            self.make_text(397, 50, 'PySnakeGame by Arup Mandal', color = (255,255,255), size = 40, a = True)
            self.make_text(300, 100, 'Telegram Group: http://t.me/iamarupmandal', color = (255,255,255), size = 20, a = True)
            self.make_text(312, 130, 'GitHub: https://github.com/arupmandal', color = (255,255,255), size = 20, a = True)
            self.make_text(295, 160, 'Feedback & Help on my Telegram.', color = (255,255,255), size = 20, a = True)
            self.make_text(348, 190, 'EMail: arupmandal731@gmail.com', color = (255,255,255), size = 20, a = True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
